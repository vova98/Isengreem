...
char** bufferToArray(const char* buffer)
{
	if (buffer == NULL)
		Nerror = MEMERR;
	PrintError(__LINE__);
	...
	*countOfString = 0;
	...
	char** text = new char*[*countOfString+1];
	int j = 0, iOld = 0;
	i = 0;
	text[j++] = (char*)buffer;
	while (buffer[i] != '\0')
	if (buffer[i++] == '\r')
	{
		...
		text[j][strlen(text[j])] = '\0';
		j++;
	}
	return text;
}

const char* PrintFile(const char* buffer, FILE* fileout)
{
	if (buffer == NULL)
		Nerror = MEMERR;
	PrintError(__LINE__);
